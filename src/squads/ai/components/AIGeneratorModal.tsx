...
// Modal for AI-powered carousel generation
...
import { useLanguage } from '@/squads/core';
import type { GeneratedCarousel, CarouselPattern } from '../types';

// Pattern visual configuration
...
> = {
  problem_solution: {
    ...
    description: {
      de: 'Problem aufzeigen, Lösung präsentieren',
      en: 'Show problem, present solution',
    },
  },
  listicle: {
    icon: List,
    ...
    description: {
      de: 'Tipps & Tricks als Liste',
      en: 'Tips & tricks as a list',
    },
  },
  story: {
    icon: BookOpen,
    ...
    description: {
      de: 'Fesselnde Geschichte erzählen',
      en: 'Tell an engaging story',
    },
  },
  comparison: {
    icon: GitCompare,
    ...
    description: {
      de: 'Optionen gegenüberstellen',
      en: 'Compare options side by side',
    },
  },
  myth_busting: {
    icon: Target,
    ...
    description: {
      de: 'Mythen entlarven & aufklären',
      en: 'Bust myths & educate',
    },
  },
};
...
interface AIGeneratorModalProps {
  isOpen: boolean;
  onClose: () => void;
  onGenerated: (carousel: GeneratedCarousel) => void;
}
...
export const AIGeneratorModal: React.FC<AIGeneratorModalProps> = ({
  ...
  const { generateCarousel, getPatterns, isGenerating, progress, error, clearError } =
    useAIGeneration();

  const [hypothesis, setHypothesis] = useState('');
  const [pattern, setPattern] = useState('problem_solution');
  const [slideCount, setSlideCount] = useState(5);
  const [tone, setTone] = useState<'professional' | 'casual' | 'provocative'>('professional');

  const patterns = getPatterns();

  const handleGenerate = async () => {
    if (!hypothesis.trim() || hypothesis.trim().length < 10) {
      return;
    }
  ...
  const getProgressText = (): string => {
    switch (progress.stage) {
      case 'analyzing':
        return t('aiGenerator.progressAnalyzing');
      case 'generating':
        return t('aiGenerator.progressGenerating');
      case 'formatting':
        return t('aiGenerator.progressFormatting');
      case 'complete':
        return t('aiGenerator.progressComplete');
      default:
        return t('aiGenerator.progressStarting');
    }
  };
  ...
  const toneOptions = [
    {
      ...
      label: t('aiGenerator.toneProfessional'),
      emoji: '👔',
    },
    {
      id: 'casual' as const,
      label: t('aiGenerator.toneCasual'),
      emoji: '😊',
    },
    {
      id: 'provocative' as const,
      label: t('aiGenerator.toneProvocative'),
      emoji: '🔥',
    },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-2xl mx-4 bg-gradient-to-b from-[#1A1A1D] to-[#141416] rounded-3xl border border-white/10 shadow-2xl shadow-black/50 animate-scale-in overflow-hidden">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {Object.keys(patterns).map((id) => {
                const config = PATTERN_CONFIG[id] || PATTERN_CONFIG.problem_solution;
                ...
                return (
                  ...
                  >
                    <div
                      className={`absolute inset-0 bg-gradient-to-br ${config.gradient} opacity-0 group-hover:opacity-100 transition-opacity`}
                    />
                    ...
                    <div className="relative">
                      ...
                      <p className="text-[10px] text-white/40 mt-1 line-clamp-1">
                        {config.description[language as 'de' | 'en'] || config.description.en}
                      </p>
                    </div>
                  </button>
                );
            ...
            <div className="flex gap-2">
              {toneOptions.map(({ id, label, emoji }) => (
                ...
                >
                  ...
                  <span
                    className={`text-sm font-medium ${
                      tone === id ? 'text-white' : 'text-white/60'
                    }`}
                  >
                    {label}
                  </span>
        ...
        <div className="p-6 border-t border-white/5 bg-black/20">
          ...
          <p className="text-center text-xs text-white/30 mt-4">{t('aiGenerator.editHint')}</p>
        </div>
      </div>
    </div>
  );
