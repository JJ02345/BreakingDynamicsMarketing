      ...
      <nav className="fixed top-0 right-0 left-0 z-50 border-b border-white/5 bg-[#0A0A0B]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6">
          ...
          <div className="flex items-center gap-4">
            <LanguageSwitcher />
            {isAuthenticated ? (
              <button onClick={() => props.nav('dashboard')} className="btn-ghost text-sm">{t('nav.dashboard')}</button>
            ) : (
        ...
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-8 text-sm text-white/40">
              <button onClick={() => props.nav('datenschutz')} className="hover:text-white transition-colors">{t('landing.privacy')}</button>
              <button onClick={() => props.nav('agb')} className="hover:text-white transition-colors">{t('landing.terms')}</button>
              <span>© 2025</span>
            </div>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-[#0A0A0B] p-6 flex items-center justify-center">
        <div className="w-full max-w-xl card-dark p-8 animate-scale-in">
          ...
          <div className="mt-4 flex gap-3">
            <button onClick={handleNewSurvey} className="btn-secondary flex-1 justify-center">{t('editor.newSurvey')}</button>
            {isAuthenticated && <button onClick={() => props.nav('dashboard')} className="btn-ghost flex-1 justify-center">{t('nav.dashboard')}</button>}
          </div>
        </div>
      </div>
    );
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        {props.error && (
          <div className="mb-6 flex items-center gap-3 rounded-xl bg-red-500/10 border border-red-500/20 px-4 py-3 text-sm text-red-400">
            <AlertCircle className="h-4 w-4 flex-shrink-0" />
            <span>{t('dashboard.loadError')} {props.error}</span>
