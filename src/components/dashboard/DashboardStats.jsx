import React from 'react';
import { LayoutGrid, TrendingUp, Layers, Download } from 'lucide-react';
import { useLanguage } from '../../context/LanguageContext';

const DashboardStats = ({ stats }) => {
  const { t } = useLanguage();

  return (
    <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
      <div className="card-dark p-5">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-xs font-medium text-white/40 uppercase tracking-wider">{t('dashboard.totalCarousels')}</span>
          <div className="w-10 h-10 rounded-xl bg-[#FF6B35]/10 flex items-center justify-center">
            <LayoutGrid className="h-5 w-5 text-[#FF6B35]" />
          </div>
        </div>
        <p className="font-['Syne'] text-4xl font-bold text-gradient-orange">{stats.total}</p>
      </div>
      <div className="card-dark p-5">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-xs font-medium text-white/40 uppercase tracking-wider">{t('dashboard.thisWeek')}</span>
          <div className="w-10 h-10 rounded-xl bg-[#00E676]/10 flex items-center justify-center">
            <TrendingUp className="h-5 w-5 text-[#00E676]" />
          </div>
        </div>
        <p className="font-['Syne'] text-4xl font-bold text-[#00E676]">{stats.week}</p>
      </div>
      <div className="card-dark p-5">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-xs font-medium text-white/40 uppercase tracking-wider">{t('dashboard.totalSlides')}</span>
          <div className="w-10 h-10 rounded-xl bg-[#00D4FF]/10 flex items-center justify-center">
            <Layers className="h-5 w-5 text-[#00D4FF]" />
          </div>
        </div>
        <p className="font-['Syne'] text-4xl font-bold text-[#00D4FF]">{stats.totalSlides || 0}</p>
      </div>
      <div className="card-dark p-5">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-xs font-medium text-white/40 uppercase tracking-wider">{t('dashboard.exports')}</span>
          <div className="w-10 h-10 rounded-xl bg-purple-500/10 flex items-center justify-center">
            <Download className="h-5 w-5 text-purple-400" />
          </div>
        </div>
        <p className="font-['Syne'] text-4xl font-bold text-purple-400">{stats.exports || 0}</p>
      </div>
    </div>
  );
};

export default DashboardStats;
