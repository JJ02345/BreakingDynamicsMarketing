...
const App = function() {
  ...
  const [surveysError, setSurveysError] = useState(null); // FIX: Error state for surveys
  ...
  if (page === 'dashboard' && !isAuthenticated) {
    return <Landing nav={navigateTo} showLogin={() => setLoginModal(true)} user={user} loginModal={loginModal} setLoginModal={setLoginModal} onLogin={handleLogin} />;
  }
  ...
  if (page === 'editor') {
    return <Editor nav={navigateTo} user={user} onLogin={handleLogin} surveys={surveys} setSurveys={setSurveys} editSurvey={editSurvey} setEditSurvey={setEditSurvey} loadSurveys={loadSurveys} />;
  }
  ...
  const handleReg = async () => {
    ...
    if (result.success) {
      if (result.data?.user && !result.data?.session) {
        setRegError('Bitte bestätige deine E-Mail-Adresse');
      } else {
        setModal(false);
        finalize();
      }
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6">
            ...
            <button 
              onClick={handleReg} 
              disabled={!privacy || pw.length < 6 || pw !== pwConfirm} 
              className={"w-full rounded-xl px-4 py-3 text-sm font-bold uppercase " + 
                (privacy && pw.length >= 6 && pw === pwConfirm 
